		...
		// create input references for model that generate zone-specific inputs (optional)
			...
			if(m_moistureBalanceEnabled){
				...
				if (intMoistLoadsModel != nullptr) {
					...
					r.m_required = false;
					// equipment load
					r.m_name.m_name = "PersonMoistureLoad";
					internalMoistLoadsRH.push_back(r);
					// person load
					r.m_name.m_name = "PersonMoistureEnthalpyFlux";
					internalMoistLoadsRH.push_back(r);
				}
			}
										 ...
										 const std::vector<const double *> & resultValueRefs)
{
	...
	if(m_moistureBalanceEnabled) {
		...
		for (unsigned int i=0; i<m_infiltrationModelCount; ++i, it+= 2) {
			if (*it == nullptr)	{
				if(*(it+1) != nullptr)
			...
			if(*(it+1) == nullptr)
				throw IBK::Exception(IBK::FormatString("Infiltration heat flux without moisture mass flux generated "
													   "for zone id=%1.").arg(m_id), FUNC_ID);
			m_infiltrationMoistMassfluxValueRef = *(it+1);
		}
	}
	else {
	...
	// internal loads
	for (unsigned int i=0; i<m_internalLoadsModelCount; ++i, it += 3) {
		if (*it == nullptr)	continue; // nullptr = no flux = skip
		// check against duplicate definition
		...
		m_personLoadValueRef = *(it+1);
		m_lightingLoadValueRef = *(it+2);
	...
	if(m_moistureBalanceEnabled) {
		...
		for (unsigned int i=0; i<m_internalMoistureLoadsModelCount; ++i, it += 2) {
			if (*it == nullptr)	continue; // nullptr = no flux = skip
			// check against duplicate definition
			if (m_personMoistureLoadValueRef != nullptr ||
				m_personMoistureEnthalpyFluxValueRef != nullptr)
				throw IBK::Exception(IBK::FormatString("Duplicate internal moisture loads generated by different models "
													   "for zone id=%1.").arg(m_id), FUNC_ID);
			m_personMoistureLoadValueRef = *(it);
			// copy all other definitions
			m_personMoistureEnthalpyFluxValueRef = *(it+1);

			// now check existence of all definitions
			if (m_personMoistureLoadValueRef == nullptr &&
				m_personMoistureEnthalpyFluxValueRef !=  nullptr )	 {
				throw IBK::Exception(IBK::FormatString("Missing person moisture load result "
													   "for zone id=%1.").arg(m_id), FUNC_ID);
			}
			if (m_personMoistureEnthalpyFluxValueRef == nullptr &&
				m_personMoistureLoadValueRef !=  nullptr)	 {
				throw IBK::Exception(IBK::FormatString("Missing person moisture enthalpy flux result "
													   "for zone id=%1.").arg(m_id), FUNC_ID);
			}
		}
	}
	...
	for (unsigned int i=0; i<m_idealHeatingCoolingModelCount; ++i, it += 2) {
		if (*it == nullptr)	continue; // nullptr = no flux = skip
		// check against duplicate definition
		...
		m_idealCoolingLoadValueRef = *(it+1);
	}
...
int RoomBalanceModel::update() {
	...
	if(m_moistureBalanceEnabled)
	{
		...
		if (m_personMoistureLoadValueRef != nullptr) {
			sumMdotMoist += *m_personMoistureLoadValueRef;
		}
		if (m_personMoistureEnthalpyFluxValueRef != nullptr) {
			SumQdot += *m_personMoistureEnthalpyFluxValueRef;
		}
