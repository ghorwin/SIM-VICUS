...
/*!
 ...
 The tolerance is the maximum normalized distance, that is
 acceptable between the original curve and the smoothed curve. A value
 of 1 will reproduce the original curve pretty good, but use 0.1 or
 a little less for near perfect fit even for small curvatures of
 the original lines and when exporting vector graphics.
...
/*!
 ...
 \param numPoints Maximum for the number of points passed to the algorithm.
                  A value of 1000 will typically be a good compromise between
                  algorithm speed and saving of points.
...
QPolygonF QwtWeedingCurveFitter::fitCurve( const QPolygonF &points ) const
{
    ...
    // protect against empty points vector, can happen when zooming into range without data
    if (points.isEmpty())
        return fittedPoints;
