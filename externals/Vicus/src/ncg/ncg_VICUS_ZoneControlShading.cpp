...
void ZoneControlShading::readXML(const TiXmlElement * element) {
	FUNCID(ZoneControlShading::readXML);
	...
	try {
		...
		while (c) {
			...
			if (cName == "Notes")
				...
				para_t ptype;
				try {
					ptype = (para_t)KeywordList::Enumeration("ZoneControlShading::para_t", p.name);
				...
				try {
					m_category = (Category)KeywordList::Enumeration("ZoneControlShading::Category", c->GetText());
				}
		...
		throw IBK::Exception( ex, IBK::FormatString("Error reading 'ZoneControlShading' element."), FUNC_ID);
	}
	catch (std::exception & ex2) {
		throw IBK::Exception( IBK::FormatString("%1\nError reading 'ZoneControlShading' element.").arg(ex2.what()), FUNC_ID);
	}
}
...
TiXmlElement * ZoneControlShading::writeXML(TiXmlElement * parent) const {
	...
	if (m_category != NUM_C)
		TiXmlElement::appendSingleAttributeElement(e, "Category", nullptr, std::string(), KeywordList::Keyword("ZoneControlShading::Category",  m_category));
