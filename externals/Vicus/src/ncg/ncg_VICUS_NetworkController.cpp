...
void NetworkController::readXML(const TiXmlElement * element) {
	...
	try {
		...
		while (attrib) {
			...
			else if (attribName == "scheduleId")
				m_scheduleId = NANDRAD::readPODAttributeValue<unsigned int>(element, attrib);
...
TiXmlElement * NetworkController::writeXML(TiXmlElement * parent) const {
	...
	if (m_scheduleId != VICUS::INVALID_ID)
		e->SetAttribute("scheduleId", IBK::val2string<unsigned int>(m_scheduleId));
