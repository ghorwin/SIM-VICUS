...
void ZoneIdealHeatingCooling::readXML(const TiXmlElement * element) {
	FUNCID(ZoneIdealHeatingCooling::readXML);
	...
	try {
		...
		while (c) {
			...
			if (cName == "Notes")
				...
				para_t ptype;
				try {
					ptype = (para_t)KeywordList::Enumeration("ZoneIdealHeatingCooling::para_t", p.name);
		...
		throw IBK::Exception( ex, IBK::FormatString("Error reading 'ZoneIdealHeatingCooling' element."), FUNC_ID);
	}
	catch (std::exception & ex2) {
		throw IBK::Exception( IBK::FormatString("%1\nError reading 'ZoneIdealHeatingCooling' element.").arg(ex2.what()), FUNC_ID);
	}
}
