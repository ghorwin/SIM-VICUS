...
void BoundaryCondition::readXML(const TiXmlElement * element) {
	FUNCID(BoundaryCondition::readXML);
	...
	try {
		...
		throw IBK::Exception( ex, IBK::FormatString("Error reading 'BoundaryCondition' element."), FUNC_ID);
	}
	catch (std::exception & ex2) {
		throw IBK::Exception( IBK::FormatString("%1\nError reading 'BoundaryCondition' element.").arg(ex2.what()), FUNC_ID);
	}
}
