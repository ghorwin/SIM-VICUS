...
void AcousticTemplate::readXML(const TiXmlElement * element) {
	FUNCID(AcousticTemplate::readXML);
	...
	try {
		...
		throw IBK::Exception( ex, IBK::FormatString("Error reading 'AcousticTemplate' element."), FUNC_ID);
	}
	catch (std::exception & ex2) {
		throw IBK::Exception( IBK::FormatString("%1\nError reading 'AcousticTemplate' element.").arg(ex2.what()), FUNC_ID);
	}
}

TiXmlElement * AcousticTemplate::writeXML(TiXmlElement * parent) const {
	if (m_id == VICUS::INVALID_ID)  return nullptr;
	TiXmlElement * e = new TiXmlElement("AcousticTemplate");
