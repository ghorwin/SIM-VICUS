...
void ZoneControlNaturalVentilation::readXML(const TiXmlElement * element) {
	FUNCID(ZoneControlNaturalVentilation::readXML);
	...
	try {
		...
		while (c) {
			...
			if (cName == "Notes")
				...
				bool success = false;
				try {
					ScheduleType ptype = (ScheduleType)KeywordList::Enumeration("ZoneControlNaturalVentilation::ScheduleType", p.name);
					m_scheduleId[ptype] = p; success = true;
				}
		...
		throw IBK::Exception( ex, IBK::FormatString("Error reading 'ZoneControlNaturalVentilation' element."), FUNC_ID);
	}
	catch (std::exception & ex2) {
		throw IBK::Exception( IBK::FormatString("%1\nError reading 'ZoneControlNaturalVentilation' element.").arg(ex2.what()), FUNC_ID);
	}
}
...
TiXmlElement * ZoneControlNaturalVentilation::writeXML(TiXmlElement * parent) const {
	...
	for (unsigned int i=0; i<NUM_ST; ++i) {
		if (!m_scheduleId[i].name.empty()) {
			TiXmlElement::appendSingleAttributeElement(e, "IBK:IntPara", "name", m_scheduleId[i].name, IBK::val2string(m_scheduleId[i].value));
		}
	}
