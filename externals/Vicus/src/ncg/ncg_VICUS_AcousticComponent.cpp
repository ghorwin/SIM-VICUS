...
void AcousticComponent::readXML(const TiXmlElement * element) {
	...
	try {
		...
		while (attrib) {
			...
			else if (attribName == "sizeMethod")
				try {
					m_sizeMethod = (SizeMethod)KeywordList::Enumeration("AcousticComponent::SizeMethod", attrib->ValueStr());
				}
		...
		while (c) {
			...
			if (cName == "ImpactSoundValue")
				...
				para_t ptype;
				try {
					ptype = (para_t)KeywordList::Enumeration("AcousticComponent::para_t", p.name);
...
TiXmlElement * AcousticComponent::writeXML(TiXmlElement * parent) const {
	...
	if (m_sizeMethod != NUM_SM)
		e->SetAttribute("sizeMethod", KeywordList::Keyword("AcousticComponent::SizeMethod",  m_sizeMethod));
