...
//#define SUNDIALS_USE_LOOP_UNROLLING
#ifdef SUNDIALS_USE_LOOP_UNROLLING
      if (a_kj != ZERO) {
        kptr=sub_diag_k;
        ...
        for (; i < n2; i += 8) {
          jptr[i]   += a_kj * kptr[i];
          jptr[i+1] += a_kj * kptr[i+1];
          jptr[i+2] += a_kj * kptr[i+2];
          jptr[i+3] += a_kj * kptr[i+3];
          jptr[i+4] += a_kj * kptr[i+4];
          jptr[i+5] += a_kj * kptr[i+5];
          jptr[i+6] += a_kj * kptr[i+6];
          jptr[i+7] += a_kj * kptr[i+7];
        }
      }

#else /* SUNDIALS_USE_LOOP_UNROLLING */
...
#endif /* SUNDIALS_USE_LOOP_UNROLLING */
