...
InterfaceLongWaveEmission::InterfaceLongWaveEmission() :
	m_modelType(NUM_MT)
{
	std::set<int> parametersModelConstant;
	parametersModelConstant.insert( (int) P_Emissivity);
...
void InterfaceLongWaveEmission::readXML(const TiXmlElement * element) {
	...
	try {
		...
		if (! KeywordList::KeywordExists("InterfaceLongWaveEmission::modelType_t", model->Value() ) ) {
			throw IBK::Exception( IBK::FormatString(XML_READ_ERROR).arg(element->Row()).arg(
				IBK::FormatString("Invalid model id '%1'.").arg(model->Value())
				), FUNC_ID);
		}

		m_modelType = (modelType_t)KeywordList::Enumeration("InterfaceLongWaveEmission::modelType_t", model->Value());
		// read parameters
		// read sub-elements
		for (const TiXmlElement * c = element->FirstChildElement(); c; c = c->NextSiblingElement()) {
			...
			if (cname == "IBK:Parameter") {
				...
				para_t t = (para_t)KeywordList::Enumeration("InterfaceLongWaveEmission::para_t", namestr);
				...
				std::string paraUnit = KeywordList::Unit("InterfaceLongWaveEmission::para_t", t);
				if (unitstr != paraUnit) {
					try {
		...
		throw IBK::Exception(ex,
			IBK::FormatString("Error reading 'LongWaveEmission' element."),
			FUNC_ID);
	}
	catch (std::exception & ex2) {
		throw IBK::Exception( IBK::FormatString("%1\nError reading 'LongWaveEmission' element.").arg(ex2.what()), FUNC_ID);
	}
}
...
void InterfaceLongWaveEmission::writeXML(TiXmlElement * parent) const {
	...
	e->SetAttribute( "model", KeywordList::Keyword("InterfaceLongWaveEmission::modelType_t", m_modelType) );
