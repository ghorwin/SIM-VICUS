	...
	try {
		...
		while (attrib) {
			...
			if (attribName == "fmiVarName")
				...
				m_fmiValueRef = NANDRAD::readPODAttributeValue<unsigned int>(element, attrib);
			else {
		...
		if (!element->FirstChildElement("RefTypeName"))
			throw IBK::Exception( IBK::FormatString(XML_READ_ERROR).arg(element->Row()).arg(
				IBK::FormatString("Missing required 'RefTypeName' element.") ), FUNC_ID);
		...
		while (c) {
			...
			if (cName == "FmiTypeName")
				...
				m_varName = c->GetText();
			else if (cName == "RefTypeName")
				m_refTypeName = c->GetText();
			else if (cName == "VarID")
				m_varID = NANDRAD::readPODElement<unsigned int>(c, cName);
			else {
...
TiXmlElement * FMIVariableDefinition::writeXML(TiXmlElement * parent) const {
	...
	if (m_fmiValueRef != NANDRAD::INVALID_ID)
		e->SetAttribute("fmiValueRef", IBK::val2string<unsigned int>(m_fmiValueRef));
	...
	if (!m_refTypeName.empty())
		TiXmlElement::appendSingleAttributeElement(e, "RefTypeName", nullptr, std::string(), m_refTypeName);
