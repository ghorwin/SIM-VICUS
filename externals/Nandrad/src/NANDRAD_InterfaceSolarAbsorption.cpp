...
InterfaceSolarAbsorption::InterfaceSolarAbsorption() :
	m_modelType(NUM_MT)
{
	std::set<int> parametersModelConstant;
	parametersModelConstant.insert( (int) P_AbsorptionCoefficient);
...
void InterfaceSolarAbsorption::readXML(const TiXmlElement * element) {
	...
	try {
		...
		if (! KeywordList::KeywordExists("InterfaceSolarAbsorption::modelType_t", model->Value() ) ) {
			throw IBK::Exception( IBK::FormatString(XML_READ_ERROR).arg(element->Row()).arg(
		...
		m_modelType = (modelType_t)KeywordList::Enumeration("InterfaceSolarAbsorption::modelType_t", model->Value());
		...
		for (const TiXmlElement * c = element->FirstChildElement(); c; c = c->NextSiblingElement()) {
			...
			if (cname == "IBK:Parameter") {
				...
				para_t t = (para_t)KeywordList::Enumeration("InterfaceSolarAbsorption::para_t", namestr);
				...
				if ( ! IBK::map_contains ( m_modelTypeToParameterMapping[ m_modelType ], t) ){
					throw IBK::Exception( IBK::FormatString(XML_READ_ERROR).arg(c->Row()).arg(
						...
						.arg(namestr)
						.arg(KeywordList::Keyword("InterfaceSolarAbsorption::modelType_t", m_modelType))
						), FUNC_ID);
				}
				...
				std::string paraUnit = KeywordList::Unit("InterfaceSolarAbsorption::para_t", t);
				if (unitstr != paraUnit) {
					try {
		...
		throw IBK::Exception(ex,
			IBK::FormatString("Error reading 'SolarAbsorption' element."),
			FUNC_ID);
	}
	catch (std::exception & ex2) {
		throw IBK::Exception( IBK::FormatString("%1\nError reading 'SolarAbsorption' element.").arg(ex2.what()), FUNC_ID);
	}
}
...
void InterfaceSolarAbsorption::writeXML(TiXmlElement * parent) const {
	...
	e->SetAttribute( "model", KeywordList::Keyword("InterfaceSolarAbsorption::modelType_t", m_modelType) );
