...
|`Albedo`|---|Wird für die Berechnung der diffusen Sonneneinstrahlung verwendet (siehe <<Lasten_Sonneneinstrahlung>>)| 0...1 |_erforderlich_
...
Einem Sensor muss eine eindeutige ID-Nummer und die obligatorischen Parameter `Orientation` und `Inclination` gegeben werden (siehe Abschnitt <<onstruction_instances,Construction Instances>> für Details zu deren Definition).
