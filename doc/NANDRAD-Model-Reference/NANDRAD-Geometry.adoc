...
.Parameter Definition for Windows
====
[source,xml, indent=0]
----
<EmbeddedObjects>
	<EmbeddedObject id="2000" displayName="A window">
		...
		<EmbeddedObjectWindow glazingSystemID="123">
			<!-- Frame and Divider tags are optional, materialID reference a material. -->
			<Frame materialID="1001">
				<!-- Area parameter is required. -->
				<IBK:Parameter name="Area" unit="m2">3</IBK:Parameter>
			</Frame>
			<Divider materialID="1001">
				<!-- Area parameter is required. -->
				<IBK:Parameter name="Area" unit="m2">2</IBK:Parameter>
			</Divider>

			<!-- Precomputed shading: when shading file is defined in Location, than
				shaded window percentage is retrieved from this file, 
				column is identified by "embObj14" composed from object type and id.
				ConstructionInstance would be "con443".
				
				Sanity check: all objects (windows/constructions) must have a
				matching column in the file.
			-->

			<!-- Controlled Shading:  -->
			<Shading type="simple" controlID="12">
				<!-- Solar radiation load = g_rad = Fz(precomputed) * Fz(Controlled) * ( SHGC * g_dir + SHGC(90 Deg)* g_diff)

					full shaded:     Fz = 1 - (1 - 80%) * 1      = 0.8
					unshaded shaded: Fz = 1 - (1 - 80%) * 0      = 1
					partially shaded: Fz = 1 - (1 - 80%) * 0.5   = 0.9

				-->
				
				<!-- ReductionFactor = Fz  = Abminderungsfaktor für Verschattung -->
				<IBK:Parameter name="ReductionFactor" unit="%">80</IBK:Parameter>
			</Shading>
