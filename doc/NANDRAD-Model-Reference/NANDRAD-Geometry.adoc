...
[NOTE]
====
As described in section <<precomputed_shading>>, the association between provided data columns and object ID is done via identification string, composed from object type and ID number. For example, and embedded object with ID 14 would get the column header/caption _embObj14_ and a construction instance (opaque surface) with ID 29 would get _conInst29_.
====
...
Glazing systems are defined in a list besides the other database lists `Materials` and `ConstructionTypes` (see <<databases>>).

Different complexities of glazing systems are supported. The most simply one is defined with model type _Simple_ and requires only a U-value (thermal transmittance) and incidence-angle dependent SHGC value:
...
In this simple model, the SHGC value for normal incidence is used for diffuse solar radiation.
