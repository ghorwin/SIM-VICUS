...
|====================
|Name|Unit|Description|Value Range | Usage 
| `Volume` | m3 | Fluid volume inside the heat exchanger | ({nbsp}>{nbsp}0{nbsp}) | only `ThermalHydraulicNetwork`
|====================
...
| `pipePropertiesId` |  Unique ID of referenced `HydraulicNetworkPipeProperties` | ({nbsp}>{nbsp}0{nbsp})  | _optional_
...
[NOTE]
====
Verschiedene Strömungselemente sind durch die Knoten IDs `inletNodeId` und `outletNodeId` verknüpft verknüpft. Die Knoten IDs referenzieren keine Strömungselemente sondern "virtuelle" Knoten.
====
Jedes Strömungselement referenziert jeweils eine Komponente mit der `componentID`. Ist diese Komponente ein Rohr (z.B. `DynamicPipe`), müssen entsprechende Rohrparameter mit der `pipePropertiesID` referenziert werden. Diese Komponente kann dann weiter spezifiert werden über die folgenden Parameter:

[options="header",cols="18%,^ 15%,38%,^ 20%, ^ 10%",width="100%"]
|====================
|Name|Unit|Description|Value Range | Usage 
| `Length` | m | Length of pipe | ({nbsp}>{nbsp}0{nbsp}) | _optional_
|====================
