...
Dieses Modell ist ähnlich der `ConstantPressurePump`, nur dass diesmal die Druckhöhe nicht fest vorgegeben ist, sonder anhand verschiedener Regelmodelle angepasst wird.
...
Die Parameter und Attribute der `ControlledPump` Definition entsprechen denen der `ConstantPressurePump`. Der Parameter `PressureHead` wird nicht benötigt.

Die tatsächliche Pumpenleistung ergibt sich durch Anwendung des Massenstromkontrollmodells, welches vom Netzwerelement im Attribut `controlElementId` referenziert wird (siehe auch <<NetworkControlElements>>).
