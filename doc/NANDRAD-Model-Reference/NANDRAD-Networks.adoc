...
Das Model `HeatPumpIdealCarnotSourceSide` ist eine ideale Wärmepumpe mit gegebener Carnot-Effizienz, welche die Quellenseite abbildet.

TODO : Hauke, detailliert erläutern und Beispiel/Parameterliste prüfen/korrigieren

Es werden diese Parameter benötigt:
...
Das Model `HeatPumpIdealCarnotSupplySide` ist eine ideale Wärmepumpe mit gegebener Carnot-Effizienz, welche die Versorgungsseite abbildet.

TODO : Hauke, detailliert erläutern
...
<HydraulicNetworkElement id="2" inletNodeId="0" outletNodeId="101" componentId="2" 
	...
	<!-- Heat loss from element (to environment) is given via time-series.
		 This heat loss is used together with the current mass
		 flux to compute the temperature difference between
		 inlet and outlet. Then, this temperature difference
		 goes into the controller referenced via
		 controlElementId which then computes a correction
		 zeta-value (flow resistance) and effectively controls
		 the mass flux.
	-->
	<HydraulicNetworkHeatExchange modelType="HeatLossSpline">
