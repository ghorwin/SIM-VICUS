...
Die Pumpeneffizient ist als der mechanische Gesamtwirkungsgrad der Pumpe definiert. D.h. die durch Volumenstrom und Druckhöhe gegebene mechanische Arbeit entspricht diesem Anteil der Gesamtarbeit. Die Differenz der Leistungen wird als Wärmequelle dem Fluid aufgeprägt.
...
| `TemperatureConstant` | Konstante Umgebungstemperatur ist als Parameter im `HydraulicNetworkElement` gegeben | `SimplePipe`, `DynamicPipe` 
| `TemperatureDataFile` | Umgebungstemperatur ist als Zeitreihe in einer tsv-Datei gegeben (im `HydraulicNetworkElement` definiert) | `SimplePipe`, `DynamicPipe`
| `HeatFluxConstant` | Wärmestrom (positiv aus dem Element) ist als Parameter im `HydraulicNetworkElement` gegeben | `SimplePipe`, `DynamicPipe`, `HeatExchanger`
| `HeatFluxDataFile` | Wärmestrom (positiv aus dem Element) ist als Zeitreihe in einer tsv-Datei gegeben (im `HydraulicNetworkElement` definiert) | `SimplePipe`, `DynamicPipe`, `HeatExchanger`
| `HeatExchangeWithZoneTemperature` | Temperatur wird aus Zone (definiert in `HydraulicNetworkElement`) definiert | `SimplePipe`, `DynamicPipe`
|====================
...
Im XML-Tag `HydraulicNetworkElement` des jeweiligen Rohres muss der Parameter `AmbientTemperature` gesetzt sein. Siehe das Beispiel <<xml_example_flow_element>>
...
<<<<<<< HEAD
Im XML-Tag `HydraulicNetworkElement` des jeweiligen Elements muss der Parameter `HeatLoss` gesetzt sein, der den konstanten Wärmestrom aus dem Element angibt. Wärmegewinnen haben ein negatives Vorzeichen.
Siehe das Beispiel <<xml_example_flow_element>>
...
<Elements>
	<HydraulicNetworkElement id="1" inletNodeId="5" outletNodeId="6" componentId="1">
		...
		<IBK:Parameter name="AmbientTemperature" unit="C">10</IBK:Parameter>
