...
The location and climate are described in the tag `Location`. Mandatory parameters are the albedo as <<IBK:Parameter>> and either a climate file `ClimateFileName` or a location description with the <<IBK:Parameter,IBK:Parameters>> northern latitude `Latitude`, eastern longitude `Longitude` and the height above sea level `Elevation`.
