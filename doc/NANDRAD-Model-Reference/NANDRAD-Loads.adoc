...
|*Altitude*|m|later needed for specific altitude-related parameters (**TODO**)|positive double ({nbsp}>{nbsp}0.0{nbsp})|_optional_
|*Longitude*|Deg|If specified, overrides the location parameter `Longitude` stored therein (see <<location_settings>>).|[-180,180]|_optional_
|*Latitude*|Deg|If specified, they override the location parameter `Latitude` stored therein (see <<location_settings>>).|[-90,90]|_optional_
|====================
