...
1. Download and install Visual Studio 2019 Community Online Installer from: https://visualstudio.microsoft.com/de/thank-you-downloading-visual-studio/?sku=Community&rel=16# - Once downloaded, install only basic development option.
...
4. Download the VC Qt Tools addin (not really necessary for command line builds, but just in case you want to do development in VC directly): https://marketplace.visualstudio.com/items?itemName=TheQtCompany.QtVisualStudioTools2019
...
Note: On Ubuntu 18.04 LTS the default Qt version is 5.9.5, so that is the oldest Qt version we are going to support.
