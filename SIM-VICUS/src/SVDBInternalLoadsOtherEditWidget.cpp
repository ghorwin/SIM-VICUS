...
SVDBInternalLoadsOtherEditWidget::SVDBInternalLoadsOtherEditWidget(QWidget *parent) :
	...
	m_ui(new Ui::SVDBInternalLoadsOtherEditWidget)
{
	m_ui->setupUi(this);
...
SVDBInternalLoadsOtherEditWidget::~SVDBInternalLoadsOtherEditWidget() {
	delete m_ui;
}


void SVDBInternalLoadsOtherEditWidget::setup(SVDatabase * db, SVAbstractDatabaseTableModel * dbModel) {
	m_db = db;
	m_dbModel = dynamic_cast<SVDBInternalLoadsTableModel*>(dbModel);
}


void SVDBInternalLoadsOtherEditWidget::updateInput(int id) {
	m_current = nullptr; // disable edit triggers

	m_ui->labelOtherInput->setText(tr("Power:"));
	if (id == -1) {
		// clear input controls
		m_ui->lineEditName->setString(IBK::MultiLanguageString());
		m_ui->labelOtherInputUnit->setText("");
...
void SVDBInternalLoadsOtherEditWidget::on_lineEditName_editingFinished() {
	Q_ASSERT(m_current != nullptr);
...
void SVDBInternalLoadsOtherEditWidget::on_comboBoxMethod_currentIndexChanged(int index) {
	Q_ASSERT(m_current != nullptr);
	...
	m_ui->labelOtherInput->setText(VICUS::KeywordListQt::Description("InternalLoad::PowerMethod", index) + ":");
}
...
void SVDBInternalLoadsOtherEditWidget::updateLabel()
{
	...
	switch (m_current->m_powerMethod) {
		case VICUS::InternalLoad::PM_PowerPerArea: 	{
			...
			m_ui->labelOtherInputUnit->setText(tr("W/m2"));
		} break;
