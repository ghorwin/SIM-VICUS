...
void DRW_InterfaceImpl::addLayer(const DRW_Layer& data){
    ...
    if(data.color != -1){
        ...
        newLayer.m_color = QColor(DRW::dxfColors[0][data.color], DRW::dxfColors[1][data.color], DRW::dxfColors[2][data.color]);
    }

     //default color black
    if(newLayer.m_color == QColor()){
        newLayer.m_visible = false;
        newLayer.m_color = QColor(DRW::dxfColors[0][7], DRW::dxfColors[1][7], DRW::dxfColors[2][7]);
    }
