...
void SVView3D::exportView3d(IBK::Path fname) {
	...
	for (const VICUS::Surface *surf : selSurfaces) {
		...
		for ( const IBKMK::Vector3D &v : vertexes ) {
			...
			m_vertexes[r->m_id].push_back(vView3d);
		}

		unsigned int surfId = 0;
		unsigned int counter = 0;

		m_roomNames[r->m_id] = r->m_displayName;
		...
		for ( const VICUS::PlaneGeometry::triangle_t &triangle : triangles) { // mind that our surfaces have to point inwards
			...
			m_surfaces[r->m_id].push_back(sView3d);
			...
			if ( compInst.m_sideASurfaceID == s.m_id ){
				if ( comp != nullptr ) {
					...
					if ( boundCond != nullptr ) {
						m_surfaces[r->m_id].back().m_emittance = boundCond->m_longWaveEmission.m_para[NANDRAD::InterfaceLongWaveEmission::P_Emissivity].value;
					}
				}
			}
	...
	for ( std::pair<unsigned int, std::vector<view3dSurface>> surf : m_surfaces ) {

		unsigned int id = surf.first;
		const std::vector<view3dSurface> &surfaces = surf.second;
		const std::vector<view3dVertex> &vertexes = m_vertexes[surf.first];

		std::ofstream out( "../../data/export/View3D/" + m_roomNames[id].toStdString() + ".v3s" );

		if( !out.is_open() )
			throw IBK::Exception(IBK::FormatString(), FUNC_ID);

		// Title of View3D file
		out << "T\tGenerated View3D-file by SIM-VICUS for room" << m_roomNames[id].toStdString() << "\n";
		out << "!--------------------------------------\n";
