...
SVDBPipeEditDialog::SVDBPipeEditDialog(QWidget *parent) :
	QDialog(parent),
	m_ui(new Ui::SVDBPipeEditDialog)
{
	m_ui->setupUi(this);
	...
	m_dbModel = new SVDBPipeTableModel(this, SVSettings::instance().m_db);
	...
	m_ui->tableView->horizontalHeader()->setSectionResizeMode(SVDBPipeTableModel::ColId, QHeaderView::Fixed);
	m_ui->tableView->horizontalHeader()->setSectionResizeMode(SVDBPipeTableModel::ColCheck, QHeaderView::Fixed);
	m_ui->tableView->horizontalHeader()->setSectionResizeMode(SVDBPipeTableModel::ColName, QHeaderView::Stretch);
...
void SVDBPipeEditDialog::on_pushButtonSelect_clicked(){
	accept();
}


void SVDBPipeEditDialog::on_pushButtonCancel_clicked() {
	reject();
}


void SVDBPipeEditDialog::on_pushButtonClose_clicked() {
	accept();
}


void SVDBPipeEditDialog::on_toolButtonAdd_clicked() {
	// add new item
	...
	// resize ID column
	sourceIndex = m_dbModel->index(0, SVDBPipeTableModel::ColId);
...
void SVDBPipeEditDialog::on_toolButtonCopy_clicked() {
	// determine current item
	QModelIndex currentProxyIndex = m_ui->tableView->currentIndex();
	Q_ASSERT(currentProxyIndex.isValid());
	QModelIndex sourceIndex = m_proxyModel->mapToSource(currentProxyIndex);
	...
	const VICUS::NetworkPipe * comp = SVSettings::instance().m_db.m_pipes[id];
...
void SVDBPipeEditDialog::on_toolButtonRemove_clicked() {
	QModelIndex currentProxyIndex = m_ui->tableView->currentIndex();
	Q_ASSERT(currentProxyIndex.isValid());
	QModelIndex sourceIndex = m_proxyModel->mapToSource(currentProxyIndex);
	m_dbModel->deleteItem(sourceIndex);
	// last pipe removed? clear input widget
	if (m_dbModel->rowCount() == 0)
		onCurrentIndexChanged(QModelIndex(), QModelIndex());
}


void SVDBPipeEditDialog::onCurrentIndexChanged(const QModelIndex &current, const QModelIndex & /*previous*/) {
