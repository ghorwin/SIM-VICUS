	...
	m_postProcHandler(new SVPostProcHandler)
{
	...
	QFile f(":/quantities.txt");
	if (f.open(QFile::ReadOnly | QFile::Text)) {
		QTextStream strm(&f);
		QString quantityData = strm.readAll();
		try {
			SVSettings::instance().m_quantityManager.read(quantityData.toUtf8().data());
		}
		...
		IBK::IBK_Message("Error reading quantities.txt.", IBK::MSG_ERROR, FUNC_ID);
	}
	// Add pre-defined quantities that are normally generated by the solver
	IBK::Quantity qH(IBK::Quantity::FLUX, "TotalFluxHeat",
					 tr("Summation flux for energy balance equation").toUtf8().data(),
					 IBK::Unit("W/m2"), IBK::Color::fromHtml("#FF0000"));
	SVSettings::instance().m_quantityManager.addQuantity(qH);
	IBK::Quantity qM(IBK::Quantity::FLUX, "TotalFluxMoisture",
					 tr("Summation flux for moisture balance equation").toUtf8().data(),
					 IBK::Unit("kg/m2s"), IBK::Color::fromHtml("#0000FF"));
	SVSettings::instance().m_quantityManager.addQuantity(qM);
...
//	addLanguageAction("it", QString::fromUtf8("Italiano"));
	...
	// *** Style Customization ***
	QFile file(":/style/VicusUIStyle.qss");  /// \todo anyone up to creating a "dark" style?
...
void SVMainWindow::addLanguageAction(const QString &langId, const QString &actionCaption) {
	QString languageFilename = QString("%1/%2_%3.qm").arg(QtExt::Directories::translationsDir()).arg("SVphin6").arg(langId);
	if (langId == "en" || QFile(languageFilename).exists()) {
		...
		IBK::IBK_Message( IBK::FormatString("Language file '%1' missing.").arg(languageFilename.toUtf8().data()),
						  IBK::MSG_WARNING, "[SVMainWindow::addLanguageAction]");
	}
}
